...
mse2Config['moreText'] = 'Показать еще';
if (screen.width>768) {
    mse2Config['mode'] = 'button'; // Для обычной версии
} else {
...
var mSearch2 = {
    ...
    options: {
        ...
        more: '.btn_more',
        more_tpl: '<div class="more_units "><button class="btn btn_black js_btn btn_more">Показать больше</button></div>',
        ...
        switch (mse2Config['mode']) {
            case 'button':
                ...
                $(document).on('click', this.options['wrapper'] + ' ' + this.options['more'], function (e) {
                    ...
                    // Записываем позицию кнопки чтобы потом к ней вернуться
                    var positionMore = $(window).scrollTop();
                    mSearch2.addPage(function () {
                        // Возвращаем кэлбек при нажатии
                        $('html, body').animate({
                            scrollTop: positionMore || 0
                        }, 0)
    ...
    addPage: function (callback) {
        ...
        $(this.options['pagination_link']).each(function () {
            ...
            if (page > current) {
                ...
                var params = mSearch2.getFilters();
                mSearch2.load(params, callback, true);
                return false;
            }
        });
    },
